...
/**
 * Aircraft make controller
 */
class AircraftMake extends _Controller.default {constructor(...args) {super(...args);_defineProperty(this, "model",




    'AircraftMake');_defineProperty(this, "sanitizers",
  ...
  delete(request) {
    ...
    findByIdAndRemove(request.params.aircraft_make_id).
    then(() => ({
      success: true }));
   ...
   */
  single(request) {
    return this.getSingle(request, 'aircraft_make_id');
  }
   ...
   */
  update(request) {
    const id = request.params.aircraft_make_id;
