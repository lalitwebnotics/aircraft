"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = sendingMailNotifications;var _moment = _interopRequireDefault(require("moment"));
...
async function sendingMailNotifications(app, ses, sendAll = false) {
  ...
  trackProcess.then((modifiedProducts) => {
    ...
    for (var i = 0; i < modifiedProducts.rebates.length; i++) {
      ...
      let rebateModel = allRebates.filter((x) => (0, _utils.formatModelIdToString)(x._id) == (0, _utils.formatModelIdToString)(rebate))[0];
...
async function tracksNotifications(app, sendAll = false) {
  ...
  for (var i = 0; i < allTracks.length; i++) {
    ...
    let product = allProducts.filter((x) => (0, _utils.formatModelIdToString)(x._id) == (0, _utils.formatModelIdToString)(track.product))[0];

    if (product == null || product == undefined) {
      continue;
    }

    let last_notified = (0, _moment.default)(track.last_notified);
...
async function aircraftNotifications(app, sendAll = false) {
  ...
  for (var i = 0; i < aircraftLength; i++) {
    ...
    let user = UsersResultSet.filter((x) => (0, _utils.formatModelIdToString)(x.id) == (0, _utils.formatModelIdToString)(currentAircraftModel.user))[0];
    ...
    let lastNotify = (0, _moment.default)(currentAircraftModel.last_notified);

    if (!sendAll) {
      if (lastNotify == undefined) {
        lastNotify = (0, _moment.default)();
        return;
      } else {
        //none
        if (frequency == 0) {
          ...
          if (lastNotify.diff((0, _moment.default)(), "weeks") < 0) {
            return;
          }
          //monthly
        } else if (frequency == 3) {
          if (lastNotify.diff((0, _moment.default)(), "months") < 0) {
            return "";
          }
        }
      }
...
function findProductsBelongsToAircraft(aircraft, category, lastNotify, filterProductByApproved) {
  ...
  return ProductsResultSet.filter((x) => {
    ...
    let existsInCategories = x.categories.includes((0, _utils.formatModelIdToString)(category.category));

    // existsInCategories = true;

    let priceChanges = true;
    let rebateChanges = true;
    let isModified = (0, _moment.default)(x.updated_at) > lastNotify;
...
function getNewFFApprovedProducts(allProducts, user, last_notified) {
  ...
  let newCreatedProducts = allProducts.filter((x) => (0, _moment.default)(x.created_at).isSameOrAfter((0, _moment.default)(last_notified).subtract(1, "months")));
...
function getProductMediaObject(product) {
  ...
  if (product.media.length > 0) {
    media = allMedia.filter((x) => (0, _utils.formatModelIdToString)(x._id) == (0, _utils.formatModelIdToString)(product.media))[0];
  }
...
function getProductManufacturer(product) {
  return allManufacturers.filter((x) => (0, _utils.formatModelIdToString)(x._id) == (0, _utils.formatModelIdToString)(product.manufacturer))[0];
}
...
function checkIfNotificationAvailable(track, user, sendAll = false) {
  ...
  if (!sendAll) {
    ...
    if (subscription == "weekly" && last_notified.diff((0, _moment.default)(), "weeks") < 1) {
      return false;
    }

    if (subscription == "monthly" && last_notified.diff((0, _moment.default)(), "months") < 1) {
      return false;
    }
  }
...
function updateLastNotified(app, model, id) {
  app.getModel(model).findByIdAndUpdate(id + "", { "last_notified": (0, _moment.default)() }, function (err, result) {
...
function productComponentTemplate(product, type, manufacturer, media, rebate = null) {
  ...
  if (type == "faapproved_product") {
    imageCaption = 'Released <b> ' + (0, _moment.default)(product.created_at).fromNow() + '</b>';
  } else if (type == "rebate") {
...
function checkIfRebateProduct(product) {
  if (product.rebate) {
    if ((0, _moment.default)(product.rebate.expiry_date).isSameOrAfter((0, _moment.default)())) {
      return true;
    }
  } else {
    return product.categories.filter((x) => x.rebate && (0, _moment.default)(x.rebate.expiry_date).isSameOrAfter((0, _moment.default)())).length > 0;
  }
